/**
 * Auto-seed: runs the course population logic on first startup
 * when the database has courses but no shared_modules content.
 * 
 * Called from src/server.js on initial boot.
 */
const knex = require('../config/db');

async function autoSeedIfEmpty() {
  try {
    const sharedCount = await knex('shared_modules').count('* as c').first();
    if (sharedCount && Number(sharedCount.c) > 0) {
      return; // Already seeded
    }

    const courseCount = await knex('courses').count('* as c').first();
    if (!courseCount || Number(courseCount.c) === 0) {
      return; // No courses exist yet (migrations not run?)
    }

    console.log('📚 Auto-seeding course content (first run)...');
    ...
    // 2. Modules + lessons + quizzes
    const firstId = await knex('courses').orderBy('id').first().then(r => r.id);
    for (const [code, ttl] of Object.entries(MODS)) {
      const sid = smMap[code];
      const [mid] = await knex('modules').insert({
        course_id: firstId, shared_module_id: sid, title: ttl, summary: ttl,
    ...
    console.log(`   ✅ Auto-seed complete: ${mc.c} modules, lessons, quizzes, assignments created.`);
  } catch (err) {
